...
class ActivityEvent {
  ...
  /////custom created function by george
  ActivityEvent._new(this._type, this._confidence,this._timeStamp) {
    print("new ActivityEvent.fromCSVline");
  }
  ////////////////////////////////////////
...
/////custom created function by george
  factory ActivityEvent.fromCSVline(String json) {
    List<String> tokens = json.split(",");
    if (tokens.length < 2) {
      return ActivityEvent.empty();
    }
    ...
    ActivityType type = ActivityType.UNKNOWN;
    if (_activityMap.containsKey(tokens[1])) {
      type = _activityMap[tokens[1]]!;
    }
    int conf = int.tryParse(tokens.last)!;
    late DateTime timestamp=  DateTime.parse(tokens.first);
    return ActivityEvent._new(type, conf,timestamp);
  }
  //////////////////////////////////////
