...
class CircularMenu extends StatefulWidget {
  ...
  CircularMenu({
    ...
    this.ringDiameter,
    this.ringWidth,
    this.fabMargin = const EdgeInsets.all(25),
...
class _CircularMenuState extends State<CircularMenu>
  ...
  CircularMenuController controller;

  @override
  void initState() {
    super.initState();

    controller = widget.controller ?? CircularMenuController();
    ...
    rotateAnimation = Tween<double>(begin: 0.5, end: 1.0).animate(rotateCurve)
      ..addListener(() {
        setState(() {});
      });
  }
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: FloatingActionButton(
              child: Text("Covid Mode", textAlign: TextAlign.center, style: TextStyle(color: Colors.red),),
              backgroundColor: Colors.transparent,
              shape: RoundedRectangleBorder(side: BorderSide(color: Colors.black), borderRadius: BorderRadius.all(Radius.circular(50))),
              elevation: 0,
              onPressed: () {
                if (!animating && !open) {
                  _open();
    ...
    return options
        ...
        .map((index, option) {
          final double angle = options.length == 1
              ? 45.0
              : 360.0 / (options.length * 2 - 2) * (index * 2);
...
class CircularMenuController extends ValueNotifier<bool> {
  CircularMenuController({bool isOpen}) : super(isOpen ?? false);
